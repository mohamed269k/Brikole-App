...
import { MapPin, Phone, Navigation, Loader, Star } from './icons';
...
const MapSection: React.FC<MapSectionProps> = ({ t, currentLang, professionals, selectedCategory }) => {
    ...
    useEffect(() => {
        if (mapContainerRef.current && !mapRef.current) {
            mapRef.current = L.map(mapContainerRef.current).setView([31.7917, -7.0926], 6); // Centered on Morocco
    ...
    useEffect(() => {
        ...
        if (mapRef.current && professionals.length > 0) {
            ...
            // Adjust map bounds to fit all professionals
    ...
    if (!selectedCategory) {
        return (
            <div className="mt-16 text-center bg-gray-800/50 border border-gray-700 rounded-xl p-8">
                <h2 className="text-2xl md:text-3xl font-bold mb-4">{t('map_title')}</h2>
                <p className="text-gray-400">{t('map_select_category_prompt')}</p>
            </div>
        )
    }
    ...
    return (
        <div className="mt-16">
            ...
            <div className="bg-gray-800/50 border border-gray-700 rounded-xl p-4 md:p-6 flex flex-col md:flex-row gap-6 min-h-[600px]">
                ...
                <div ref={mapContainerRef} className="w-full h-[400px] md:h-auto md:w-2/3 rounded-lg" />
            </div>
        </div>
    );
};
